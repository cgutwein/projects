...
q <- paste("SELECT * FROM sensor_data")

drv <- dbDriver("PostgreSQL")
con <- dbConnect(drv, user="postgres", host="172.31.11.72", dbname="postgres", password="maXgBaNyr3w6TzT")
temp_df <- dbGetQuery(con, q)
dbDisconnect(con)
dbUnloadDriver(drv)
save(temp_df, file = "pa_db.RData")
